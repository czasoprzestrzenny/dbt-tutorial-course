...
order_item_measures AS (
	...
	FROM {{ ref('bronze_ecommerce__order_items_products') }}
	GROUP BY 1
)
...
FROM {{ ref('bronze_ecommerce__orders') }} AS od
LEFT JOIN order_item_measures AS om
	ON od.order_id = om.order_id
LEFT JOIN {{ ref('bronze_ecommerce__first_order_created') }} AS user_data
